...
{
	if (input == "nan" || input == "nanf") // This value represents undefined or impossible-to-calculate results...
	{
	...
	else if (input == "+inf" || input == "+inff") // It represents a value greater than any finite floating-point number
	{
		std::cout << "char: impossible" << std::endl;
		std::cout << "int: impossible" << std::endl;
...
void	intCase(const std::string &input)
{
	...
	if (n < MIN_INT || n > MAX_INT)
		std::cout << "Impossible: Exceeded the limits." << std::endl;
	else
...
void	doubleCase(const std::string &input)
{
	...
	if (d < MIN_DOUBLE || d > MAX_DOUBLE)
		...
		std::cout << static_cast<double>(d) << (tolerance ? ".0" : "") << std::endl;
}